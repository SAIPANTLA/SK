...
import React, { useState } from "react";
import { useParams, Link } from "react-router-dom";
import traineesData from "./traineesData";
...
const TraineeDetails = () => {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: 24, maxWidth: 960, margin: "auto" }}>
      ...
      {/* Quick stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 16, marginBottom: 32 }}>
        <StatCard label="Meeting Attendance" value={`${trainee.meetingAttendance.attendancePercentage}%`} subtext={`${trainee.meetingAttendance.attended}/${trainee.meetingAttendance.totalMeetings} meetings`} color="#4caf50" />
        <StatCard label="Overall Performance" value={`${trainee.performance}%`} subtext="Performance score" color="#ff9800" />
        <StatCard label="Assigned Tasks" value={trainee.assignedTasks} subtext="Tasks assigned" color="#3f51b5" />
        <StatCard label="Assessments Completed" value={trainee.assessments.length} subtext="Assessments taken" color="#9c27b0" />
      </div>
      ...
      <div>
        ...
        {activeTab === "overview" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            ...
            <section style={sectionStyle}>
              <h2 style={sectionTitle}>Performance Summary</h2>
              <Progress label="Overall Performance" value={trainee.performance} color="#ff9800" />
              <Progress label="Meeting Attendance" value={trainee.meetingAttendance.attendancePercentage} color="#4caf50" />
            </section>
          </div>
        )}
              ...
              return (
                <div key={i} style={taskCardStyle}>
                  <div style={taskCardHeader}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 18, fontWeight: "bold", color: "#4caf50" }}>{assessment.score}/{assessment.maxScore}</div>
                      <div style={{ fontSize: 12, color: "#666" }}>{percent}%</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", fontSize: 12, color: "#666", gap: 4 }}>
        ...
        {activeTab === "meetings" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 16, marginBottom: 32, textAlign: "center" }}>
              <MeetingStat label="Total Meetings" value={trainee.meetingAttendance.totalMeetings} />
              <MeetingStat label="Attended" value={trainee.meetingAttendance.attended} color="#4caf50" />
              <MeetingStat label="Late" value={trainee.meetingAttendance.late} color="#ff9800" />
              <MeetingStat label="Absent" value={trainee.meetingAttendance.absent} color="#f44336" />
            </div>

            <div>
              {trainee.recentMeetings.map((meeting, i) => (
                <div key={i} style={meetingCardStyle}>
                  <div>
                    <h4 style={{ margin: 0 }}>{meeting.title}</h4>
                    <p style={{ margin: 0, color: "#666" }}>{meeting.date}</p>
                  </div>
...
const StatCard = ({ label, value, subtext, color }) => (
  <div style={{ padding: 16, border: "1px solid #ddd", borderRadius: 8, textAlign: "center" }}>
    ...
    <div style={{ fontSize: 14, color: "#666" }}>{subtext}</div>
    <div style={{ marginTop: 8, fontWeight: "600" }}>{label}</div>
  </div>
);

const MeetingStat = ({ label, value, color = "#333" }) => (
  <div>
    <div style={{ fontSize: 24, fontWeight: "bold", color }}>{value}</div>
...
const Progress = ({ label, value, color }) => (
  <div style={{ marginBottom: 16 }}>
    <label style={{ display: "block", marginBottom: 4 }}>{label}: {value}%</label>
    <div style={{ background: "#eee", borderRadius: 4, height: 12 }}>
      <div style={{ width: `${value}%`, height: "100%", backgroundColor: color, borderRadius: 4, transition: "width 0.3s ease" }} />
    </div>
  </div>
);

/* ========== Styles ========== */
const sectionStyle = { border: "1px solid #ddd", padding: 16, borderRadius: 8 };
const sectionTitle = { marginBottom: 12 };
const taskCardStyle = { border: "1px solid #ddd", borderRadius: 8, padding: 16, marginBottom: 16 };
const taskCardHeader = { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 };
const taskFooter = { display: "flex", justifyContent: "space-between", fontSize: 12, color: "#666" };
const meetingCardStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid #ddd", padding: 12, borderRadius: 8, marginBottom: 12 };
