...
import { FiUsers, FiUserCheck, FiLayers, FiClipboard, FiMessageCircle } from "react-icons/fi";
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {summary.map((item) => (
          ...
          >
            <div className="text-4xl text-white drop-shadow-lg">{item.icon}</div>
            <div>
              <div className="text-md font-semibold text-white mb-1">{item.title}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Performance Trend */}
        <div className="bg-white rounded-xl shadow-md p-6 flex flex-col items-center min-h-[300px]">
          <span className="text-lg font-bold text-blue-600 mb-2">Trainee Performance Trend</span>
        ...
        {/* Batch Distribution */}
        <div className="bg-white rounded-xl shadow-md p-6 flex flex-col items-center min-h-[300px]">
          <span className="text-lg font-bold text-teal-600 mb-2">Batch Distribution by Domain</span>
      ...
      {/* Recent Activity */}
      <div className="bg-gradient-to-br from-blue-50 to-purple-100 rounded-xl shadow-md p-6">
        <span className="text-lg font-bold text-purple-700 mb-4 block">Recent Activity</span>
        <ul className="space-y-3">
          {recentActivity.map((item, idx) => (
            <li key={idx} className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className={`inline-block w-2 h-2 rounded-full ${statusColors[item.status]}`}></span>
